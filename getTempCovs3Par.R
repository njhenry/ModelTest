...
getTempCovsDate3Par <- function(locs, obsdates, folderpaths, endpaths, covnames=NULL, byMonth=rep(TRUE, length(folderpaths)),
  ...
  for(i in 1:length(folderpaths)){
    ...
    if(byMonth[i]){
      ...
      ##format has changed, so if that hasn't worked 
      ##need to use new format
      if(length(file.dates) == 0){
        file.dates <- as.Date(regmatches(file.names, regexpr("[0-9]{4}\\.[0-9]{3}", file.names)), format="%Y.%j")
      }
    ...
    for(k in 1:length(obsdates.use)){
      ...
      #file before date
      if(length(which.min(nonneg(obsdate - file.dates))) == 0){
        # print(obsdate)
        #print(i)
      }
      if(length(which.min(nonneg(file.dates - obsdate))) == 0){
        #print(obsdate)
        #print(i)
        #print("two")
        nearestFile2[2*k-1] <- which.min(nonneg(obsdate - file.dates)) 
        nearestFile2[2*k] <- 1
        weights <- rbind(weights, c(1, 0))
      }else if(min(nonneg(obsdate - file.dates), na.rm=T) == 0){
        nearestFile2[2*k-1] <- which.min(nonneg(obsdate - file.dates)) 
        nearestFile2[2*k] <- 1
        weights <- rbind(weights, c(1, 0))
      }else{
    ...
    cov.vals.list <- foreach(l=1:length(nonempty),
    ...
    for(l in 1:length(nonempty)){
      print(l)
      outCovVals[nearestIndex2[[nonempty[l]]]] <- outCovVals[nearestIndex2[[nonempty[l]]]] + cov.vals.list[[l]]
    }   
...
if(runtests){
  ...
  folderpaths <- list("Z:/mastergrids/MODIS_Global/MOD11A2_v6_LST/LST_Day/1km/Monthly/",
                      "Z:/mastergrids/MODIS_Global/MCD43D6_v6_BRDF_Reflectance/TCB_v6/1km/8-Daily/",
                      "Z:/mastergrids/MODIS_Global/MCD43D6_v6_BRDF_Reflectance/EVI_v6/1km/8-Daily/")
  ...
  covs <- getTempCovsDate3Par(locs, obsdates, folderpaths, endpaths, covnames, byMonth,
                                  timelags)
}
